...
public class MonoLibNeoForge {
  ...
  public MonoLibNeoForge(final IEventBus modEventBus) {
    ...
    NeoForge.EVENT_BUS.addListener((Consumer<AddServerReloadListenersEvent>) event -> {
      event.addListener(MonoLib.identifier(Constants.MOD_ID), new ResourceReloadListener());
    });

    if (FMLLoader.getCurrent().getDist() == Dist.CLIENT) {
      new MonoLibClientNeoForge(EVENT_BUS);
    }
  }
